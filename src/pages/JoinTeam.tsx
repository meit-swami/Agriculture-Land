...
const JoinTeam = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.name.trim() || !form.phone.trim()) { toast({ title: t('कृपया नाम और फ़ोन भरें', 'Please fill name and phone'), variant: 'destructive' }); return; }
    if (!/^[6-9]\d{9}$/.test(form.phone.trim())) { toast({ title: t('सही फ़ोन नंबर डालें', 'Enter a valid phone number'), variant: 'destructive' }); return; }
    setSubmitting(true);
    const { error } = await supabase.from('team_applications' as any).insert({
      name: form.name.trim().slice(0, 100), phone: form.phone.trim(),
      state: form.state || '', district: form.district || '',
      experience: form.experience.trim().slice(0, 200), message: form.message.trim().slice(0, 500),
    });
    setSubmitting(false);
    if (error) { toast({ title: t('फॉर्म भेजने में समस्या', 'Failed to submit'), variant: 'destructive' }); return; }
      ...
      <div className="container mx-auto px-4 py-8 max-w-2xl">
        <div className="bg-gradient-to-br from-primary to-secondary rounded-2xl p-6 md:p-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-xl md:text-2xl font-extrabold text-primary-foreground">{t('हमारी टीम बनें', 'Become Our Team')}</h1>
          </div>
          <p className="text-primary-foreground/80 text-sm mb-6">{t('अपनी जानकारी भरें, हम आपसे जल्द संपर्क करेंगे', 'Fill your details, we will contact you soon')}</p>
